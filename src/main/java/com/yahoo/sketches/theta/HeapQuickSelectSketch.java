...
class HeapQuickSelectSketch extends HeapUpdateSketch {
	...
	HeapQuickSelectSketch(final int lgNomLongs, final long seed, final float p, final ResizeFactor rf,
			final int preambleLongs, final Family family) {
	 ...
	 *
	 * @param lgNomLongs
	 *            <a href="{@docRoot}/resources/dictionary.html#lgNomLogs">See
	 *            lgNomLongs</a>.
	 * @param seed
	 ...
	 *            Resize Factor</a>
	 * @param unionGadget
	 *            true if this sketch is implementing the Union gadget function.
	...
	/**
	 * Heapify a sketch from a Memory UpdateSketch or Union object containing sketch
	 * data.
	 * 
	 * @param srcMem
	...
	static HeapQuickSelectSketch heapifyInstance(final Memory srcMem, final long seed) {
		...
		final ResizeFactor myRF = ResizeFactor.getRF((srcMem.getByte(PREAMBLE_LONGS_BYTE) >>> LG_RESIZE_FACTOR_BIT)); // byte
																														// 0
		...
		if (lgNomLongs < MIN_LG_NOM_LONGS) {
			throw new SketchesArgumentException("Possible corruption: Current Memory lgNomLongs < min required size: "
		...
		if (curCapBytes < minReqBytes) {
			throw new SketchesArgumentException("Possible corruption: Current Memory size < min required size: "
		...
		if ((lgArrLongs <= lgNomLongs) && (theta < p)) {
			throw new SketchesArgumentException(
					"Possible corruption: Theta cannot be < p and lgArrLongs <= lgNomLongs. " + lgArrLongs + " <= "
							+ lgNomLongs + ", Theta: " + theta + ", p: " + p);
		}
	...
	private final void quickSelectAndRebuild() {
		...
		// now we rebuild to clean up dirty data, update count, reconfigure as a hash
		// table
	 ...
	 *
	 * @param lgNomLongs
	 *            <a href="{@docRoot}/resources/dictionary.html#lgNomLongs">See
	 *            lgNomLongs</a>.
	 * @param lgArrLongs
	 *            <a href="{@docRoot}/resources/dictionary.html#lgArrLongs">See
	 *            lgArrLongs</a>.
