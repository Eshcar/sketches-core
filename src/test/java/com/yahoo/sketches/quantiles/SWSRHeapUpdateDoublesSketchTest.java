...
public class SWSRHeapUpdateDoublesSketchTest {
...
//    }
  ...
  @Test
  public void checkMyGetQuantile() {
    int k = PreambleUtil.DEFAULT_K;
    int n = 1000000;
    SWSRHeapUpdateDoublesSketch qs = myBuildAndLoadQS(k, n, 0);
...
//    double[] splitPoints = result.getQuantiles(phiArr);
    ...
    for (int q = 1; q <= 99; q++) {
      ...
      double nominal = (1e6 * q) / 100.0;
      double reported = qs.getQuantile(q / 100.0);
      assertTrue(reported >= (nominal - 10000.0));
      assertTrue(reported <= (nominal + 10000.0));
    }
...
//    qs.getQuantile(frac);
  ...
  //cannot pass this test -> the representation of bit pattern has changed.
  
//  @Test
//  public void checkConstructAuxiliary() {
//    for (int k = 2; k <= 32; k *= 2) {
//      SWSRHeapUpdateDoublesSketch qs = SWSRHeapUpdateDoublesSketch.newInstance(k);
...
//  }
  ...
  public void checkBigMinMax () {
    ...
    SWSRHeapUpdateDoublesSketch qs1 = SWSRHeapUpdateDoublesSketch.newInstance(k);
    SWSRHeapUpdateDoublesSketch qs2 = SWSRHeapUpdateDoublesSketch.newInstance(k);
    SWSRHeapUpdateDoublesSketch qs3 = SWSRHeapUpdateDoublesSketch.newInstance(k);
...
//
//    SWSRHeapUpdateDoublesSketch qs2 = SWSRHeapUpdateDoublesSketch.heapifyInstance(srcMem);
//    s = qs2.toString();
//    println(s);
//  }
...
//    println(fracArr[3]+", "+ (fracArr[3]-0.1));
  ...
  public void checkMemTooSmall1() {
    ...
    SWSRHeapUpdateDoublesSketch.heapifyInstance(mem);
    fail();
    //qs2.getQuantile(0.5);
  }
  ...
  public void checkSerVer() {
    DoublesUtil.checkDoublesSerVer(0, SWSRHeapUpdateDoublesSketch.MIN_HEAP_DOUBLES_SER_VER);
  }
  ...
  public void checkMemCapacityException() {
    ...
    SWSRHeapUpdateDoublesSketch.checkHeapMemCapacity(k, n, false, serVer, badCapBytes);
  }
  ...
  public void checkBufAllocAndCap() {
    ...
    SWSRHeapUpdateDoublesSketch.checkHeapMemCapacity(k, n, false, 1, memCapBytesV1);
    SWSRHeapUpdateDoublesSketch.checkHeapMemCapacity(k, n, false, serVer, memCapBytes - 1); //corrupt
  }
  ...
  static void testDownSampling(int bigK, int smallK) {
    SWSRHeapUpdateDoublesSketch sketch1 = SWSRHeapUpdateDoublesSketch.newInstance(bigK);
    SWSRHeapUpdateDoublesSketch sketch2 = SWSRHeapUpdateDoublesSketch.newInstance(smallK);
    for (int i = 127; i >= 1; i--) {
      sketch1.update (i);
      sketch2.update (i);
    }
    SWSRHeapUpdateDoublesSketch downSketch =
        (SWSRHeapUpdateDoublesSketch)sketch1.downSample(sketch1, smallK, null);
    println (LS+"Sk1"+LS);
    String s1, s2, down;
...
//we do not support merge yet --> -> the representation of bit pattern has changed.
//  @Test
...
//    SWSRHeapUpdateDoublesSketch sketch1 = SWSRHeapUpdateDoublesSketch.newInstance(8);
//    SWSRHeapUpdateDoublesSketch sketch2 = SWSRHeapUpdateDoublesSketch.newInstance(2);
...
//      SWSRHeapUpdateDoublesSketch bigSketch = SWSRHeapUpdateDoublesSketch.newInstance(8);
//      for (int i1 = 1; i1 <= n1; i1++ ) {
//        bigSketch.update(i1);
//      }
//      for (int n2 = 0; n2 < 50; n2++ ) {
//        SWSRHeapUpdateDoublesSketch directSketch = SWSRHeapUpdateDoublesSketch.newInstance(2);
//        for (int i1 = 1; i1 <= n1; i1++ ) {
...
//        }
//        SWSRHeapUpdateDoublesSketch smlSketch = SWSRHeapUpdateDoublesSketch.newInstance(2);
//        for (int i2 = 1; i2 <= n2; i2++ ) {
//          smlSketch.update(i2);
//        }
...
//    }
  ...
  private static void checksForImproperK(final int k) {
    ...
    try {
      SWSRHeapUpdateDoublesSketch.newInstance(k);
      fail(s);
    } catch (SketchesArgumentException e) {
      //pass
    }
   ...
   */
  private static double sumOfSamplesInSketch(SWSRHeapUpdateDoublesSketch sketch) {
  ...
  static SWSRHeapUpdateDoublesSketch myBuildAndLoadQS(int k, int n, int startV) {
	  SWSRHeapUpdateDoublesSketch qs = SWSRHeapUpdateDoublesSketch.newInstance(k);
	    for (int i=1; i<=n; i++) {
	      qs.update(startV + i);
	    }
